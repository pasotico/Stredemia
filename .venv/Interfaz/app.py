...
st.markdown("Responde las preguntas con honestidad. "
            "El resultado es orientativo y no reemplaza atención profesional.")
st.divider()
...
s8  = st.select_slider("Fatiga crónica (cansancio permanente)", options=opciones_sisco)
s9  = st.select_slider("Sentimientos de depresión y tristeza", options=opciones_sisco)
s10 = st.select_slider("Ansiedad, angustia o desesperación",   options=opciones_sisco)
s11 = st.select_slider("Problemas de concentración",           options=opciones_sisco)
s12 = st.select_slider("Sentimiento de agresividad o irritabilidad", options=opciones_sisco)
s13 = st.select_slider("Conflictos o tendencia a discutir",    options=opciones_sisco)
s14 = st.select_slider("Desgano para realizar las labores escolares", options=opciones_sisco)
st.divider()
...
s15 = st.select_slider("Me concentro en resolver la situación que me preocupa", options=opciones_sisco)
s16 = st.select_slider("Establezco soluciones concretas para resolver lo que me preocupa", options=opciones_sisco)
s17 = st.select_slider("Analizo lo positivo y negativo de las soluciones posibles", options=opciones_sisco)
s18 = st.select_slider("Mantengo el control sobre mis emociones para que no me afecte el estrés", options=opciones_sisco)
s19 = st.select_slider("Recuerdo situaciones similares anteriores y cómo las resolví", options=opciones_sisco)
s20 = st.select_slider("Elaboro un plan para enfrentar lo que me estresa y lo ejecuto", options=opciones_sisco)
s21 = st.select_slider("Trato de obtener lo positivo de la situación que me preocupa", options=opciones_sisco)
